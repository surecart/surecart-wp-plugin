...
export function addFormChild( name ) {
	cy.get(
		'[data-type="checkout-engine/form"] [data-type^="checkout-engine/field"]'
	)
		.first()
		.click( { force: true } );
	cy.get( '.block-editor-block-settings-menu' ).click();
	cy.get( '.components-popover__content button' )
		.contains( /insert after/i )
		.click( { force: true } );
	cy.get(
		'[data-type="checkout-engine/form"] [data-type="core/paragraph"]'
	).click( { force: true } );

	cy.get( '.edit-post-header-toolbar' )
		.find( '.edit-post-header-toolbar__inserter-toggle' )
		.click( { force: true } );
	cy.get( '.block-editor-inserter__search' ).click().type( name );

	cy.get(
		'.block-editor-inserter__content .editor-block-list-item-checkout-engine-field-' +
			name
	)
		.first()
		.click( { force: true } );
	cy.get( `[data-type="checkout-engine/field-${ name }"]` )
		.should( 'exist' )
		.click( { force: true } );
}
...
export function disableGutenbergFeatures() {
	getWindowObject().then( ( safeWin ) => {
		if ( ! safeWin.wp || ! safeWin.wp.data ) return;
		// Enable "Top Toolbar"
		if (
			! safeWin.wp.data
...
/**
 * From inside the WordPress editor open the checkout-engine Gutenberg editor panel
 *
 * @param {string}  blockName   The name to find in the block inserter
 *                              e.g 'core/image' or 'checkout-engine/accordion'.
...
export function savePage() {
	...
	cy.get( '.components-editor-notices__snackbar', {
		timeout: 120000,
	} ).should( 'not.be.empty' );
...
export const upload = {
	 ...
	 *                           imageReplaceFlow works with checkout-engine Galleries: Carousel, Collage, Masonry, Offset, Stacked.
	 */
	imageReplaceFlow: ( blockName ) => {
		...
		cy.get( '.checkout-engine-gallery-item__button-replace' ).should(
			'not.exist'
		);
		...
		cy.get( '.checkout-engine-gallery-item__button-replace' ).click( {
			force: true,
		} );
...
export function setColorSetting( settingName, hexColor ) {
	...
	cy.get( '.components-base-control__field' )
		...
		.then( ( $subColorPanel ) => {
			// < WP 5.9
			if (
				Cypress.$( '.components-color-palette__custom-color' )
					.length === 0
			) {
				cy.get(
					Cypress.$( $subColorPanel ).closest(
					...
					.click();
				cy.get(
					'.components-color-picker__inputs-field input[type="text"]'
				)
					.clear()
					.type( hexColor );
				cy.get(
				...
				cy.get(
					Cypress.$( $subColorPanel ).closest( '.components-flex' )
				)
...
export function addCustomBlockClass( classes, blockID = '' ) {
	...
	cy.get(
		'[class*="-visual-editor"] .wp-block[data-type="checkout-engine/' +
			blockID +
			'"]'
	)
		.last()
...
export function openEditorSettingsModal() {
	...
	cy.get( '.checkout-engine-settings-modal input[type="checkbox"]' ).should(
		'have.length',
		6
	);
}
