...
export function addFormChild(name) {
	cy.get('[data-type="surecart/form"] [data-type^="surecart/field"]')
		.first()
		.click({ force: true });
	...
	cy.get('.components-popover__content button')
		...
		.click({ force: true });
	cy.get('[data-type="surecart/form"] [data-type="core/paragraph"]').click({
		force: true,
	});
	...
	cy.get(
		'.block-editor-inserter__content .editor-block-list-item-surecart-field-' +
			name
	)
		.first()
		.click({ force: true });
	cy.get(`[data-type="surecart/field-${name}"]`)
		.should('exist')
		.click({ force: true });
}
...
/**
 * From inside the WordPress editor open the surecart Gutenberg editor panel
 *
 * @param {string}  blockName   The name to find in the block inserter
 *                              e.g 'core/image' or 'surecart/accordion'.
...
export const upload = {
	 ...
	 *                           imageReplaceFlow works with surecart Galleries: Carousel, Collage, Masonry, Offset, Stacked.
	 */
	imageReplaceFlow: (blockName) => {
		...
		cy.get('.surecart-gallery-item__button-replace').should('not.exist');

		cy.get(`[class*="-visual-editor"] [data-type="${blockName}"] img`)
			.first()
			.click({ force: true });

		cy.get('.surecart-gallery-item__button-replace').click({
			force: true,
		});
...
export function setColorSetting(settingName, hexColor) {
	...
	cy.get('.components-base-control__field')
		...
		.then(($subColorPanel) => {
			// < WP 5.9
			if (
				Cypress.$('.components-color-palette__custom-color').length ===
				0
			) {
				cy.get(
...
export function addCustomBlockClass(classes, blockID = '') {
	...
	cy.get(
		'[class*="-visual-editor"] .wp-block[data-type="surecart/' +
			blockID +
			'"]'
	)
		.last()
...
export function openEditorSettingsModal() {
	...
	cy.get('.surecart-settings-modal input[type="checkbox"]').should(
		'have.length',
		6
	);
}
