...
module.exports.getUserConfig = ( file, whitelisted = false ) => {
	...
	if ( ! fs.existsSync( userConfigPath ) ) {
		console.error(
			'\x1B[31mCould not find your config.json file. Please make a copy of config.json.dist and adjust as needed.\x1B[0m'
		);
		process.exit( 1 );
	}
	...
	try {
		...
		console.error(
			'\x1B[31mCould not parse your config.json file. Please make sure it is a valid JSON file.\x1B[0m'
		);
		process.exit( 1 );
	}
...
module.exports.filehashFilter = ( file ) =>
	`[name].${ module.exports.filehash( file ).substr( 0, 10 ) }.[ext]`;
