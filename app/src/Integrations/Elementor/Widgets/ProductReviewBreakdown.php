...
class ProductReviewBreakdown extends \Elementor\Widget_Base {
	...
	protected function content_template() {
		?>
		<#
		var starSize = settings.star_size.size || 20;
		var columns = parseInt(settings.columns) || 1;
		var rowGap = settings.row_gap.size || 20;
		var columnGap = settings.column_gap.size || 20;
		...
		#>
		<div class="wp-block-surecart-product-review-breakdown">
			<div class="sc-star-bars sc-star-bars--columns-{{ columns }}" style="display: flex; flex-direction: column; flex-wrap: wrap; row-gap: {{ rowGap }}px; column-gap: {{ columnGap }}px; align-content: flex-start; {{ maxHeight }}">
				<# for (var star = 5; star >= 1; star--) {
					var count = breakdownData[star];
					var percentage = (count / total) * 100;
					var widthStyle = getWidthStyle(columns, columnGap);
				#>
					<a href="#" class="sc-star-row" onclick="event.preventDefault();" style="display: flex; align-items: center; gap: 8px; text-decoration: none; color: inherit; cursor: pointer; transition: opacity 0.2s ease; {{ widthStyle }}">
						<div class="sc-star-label" style="display: flex; align-items: center; gap: 4px;">
							{{ star }}
						...
						<div class="sc-bar-wrap" style="flex: 1; height: 8px; border-radius: 4px; overflow: hidden; position: relative; min-width: 100px;">
							<div class="sc-bar-fill" style="height: 100%; border-radius: 4px; width: {{ percentage }}%; transition: width 0.3s ease;"></div>
						</div>
						<div class="sc-count" style="text-align: right;">{{ count }}</div>
					</a>
				<# } #>
			</div>
		</div>
