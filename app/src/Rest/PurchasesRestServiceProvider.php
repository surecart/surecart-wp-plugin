...
class PurchasesRestServiceProvider extends RestServiceProvider implements RestServiceInterface {
	 ...
	 * @var string
	 */
	protected $controller = PurchasesController::class;
	...
	public function registerRoutes() {
		register_rest_route(
			...
			$this->endpoint . '/(?P<id>\S+)/revoke/',
			[
				[
					'methods'             => \WP_REST_Server::EDITABLE,
					'callback'            => $this->callback( $this->controller, 'revoke' ),
		...
		register_rest_route(
			"$this->name/v$this->version",
			$this->endpoint . '/(?P<id>\S+)/invoke/',
			[
				[
					'methods'             => \WP_REST_Server::EDITABLE,
					'callback'            => $this->callback( $this->controller, 'invoke' ),
					'permission_callback' => [ $this, 'update_item_permissions_check' ],
				],
				// Register our schema callback.
				'schema' => [ $this, 'get_item_schema' ],
