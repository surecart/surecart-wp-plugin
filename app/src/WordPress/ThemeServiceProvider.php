...
class ThemeServiceProvider implements ServiceProviderInterface
{
    ...
    {
        $cartMenuId = get_option('surecart_cart_menu_selected_id', null);
        $cartMenuEnabled = (bool) get_option('sc_cart_menu_button_enabled', false);

        if (!$cartMenuEnabled || $cartMenuId != $args->menu->term_id) {
            return $items;
        }

        $cartMenuAlignment = get_option('surecart_cart_menu_alignment', 'left');
        $form = \SureCart::forms()->getDefault();
        $mode = Form::getMode($form->ID);

        $menu = "<li><a href='#'><sc-cart-button form-id='" . esc_attr($form->ID) . "' mode='" . esc_attr($mode) . "'/></a></li>";

        if ($cartMenuAlignment == 'right') {
            $items = $menu . $items;
        } else {
