...
if ( ! function_exists( 'checkout_engine_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function checkout_engine_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'checkout_engine_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function checkout_engine_should_load_wpemerge( $name, $min, $max ) {
		$loaded = checkout_engine_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'checkout_engine' );
				?>
...
if ( ! function_exists( 'checkout_engine_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function checkout_engine_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = checkout_engine_get_loaded_wpemerge();
