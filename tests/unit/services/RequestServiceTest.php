...
class RequestServiceTest extends CheckoutEngineUnitTestCase
{
	...
	public function test_makesLiveRequests()
	{
		$mock_service = $this->getMockBuilder(RequestService::class)
			->setMethods(['remoteRequest', 'getToken'])
			->getMock();

		$mock_service->expects($this->once())
            ->method('getToken')
			->with('live')
			->willReturn('live_token');
		...
		$mock_service->makeRequest('test-endpoint', []);
	}

	public function test_makesTestRequests()
	{
		$mock_service = $this->getMockBuilder(RequestService::class)
			->setMethods(['remoteRequest', 'getToken'])
			->getMock();
		...
		$mock_service->makeRequest('test-endpoint', [], 'test');
	}
}
