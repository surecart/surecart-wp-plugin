				...
				<svg className="star-svg" viewBox="0 0 24 24" width={size} height={size}>
					{isHalf ? (
						<>
							...
							<polygon points={points} clipPath={`url(#${clipId})`} fill={fill_color || 'var(--sc-color-primary-500)'} />
							<polygon points={points} fill="none" stroke={fill_color || 'var(--sc-color-primary-500)'} strokeWidth="1" />
						</>
					) : isFull ? (
						<polygon points={points} fill={fill_color || 'var(--sc-color-primary-500)'} stroke={fill_color || 'var(--sc-color-primary-500)'} strokeWidth="1" />
					) : (
						<polygon points={points} fill="none" stroke={'var(--sc-color-gray-300)'} strokeWidth="1" />
					)}
				</svg>
			</div>
		);
