...
export default function PostTemplateForm({
	...
	const selected = templates.find(
		(template) => template.id === upsell?.metadata?.wp_template_id
	);
	...
	return (
		...
		>
			<InspectorPopoverHeader
				title={__('Template')}
				help={__(
					'Templates define the way this upsell is displayed when viewing your site.'
				)}
				actions={
					canCreate && [
						{
			...
			<SelectControl
				...
				hideLabelFromVision
				label={__('Template')}
				value={selected?.id || 'surecart/surecart//single-upsell'}
				options={options}
				onChange={(slug) => {
					updateUpsell({
						metadata: {
