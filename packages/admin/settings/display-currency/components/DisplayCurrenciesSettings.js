...
export default function DisplayCurrenciesSettings() {
			...
			wrapperTag="div"
				...
				<ScSelect
					...
					choices={(scData?.supported_currencies || []).map(
						({ name, symbol, currency: value, flag: icon }) => ({
							label: `${name} (${symbol})`,
							value,
							icon,
						})
					...
					`}
				>
					<ScButton type="primary" slot="trigger" busy={!hasResolved}>
