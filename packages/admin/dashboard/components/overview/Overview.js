...
export default () => {
	...
	function getTotalOrdersData(ordersStates) {
		...
		ordersStates.data.map((states) => {
			...
			reportAverage =
				reportAverage + parseInt(states.average_amount / 100);
	...
	function getDatesArray(startDate, endDate, reportBy) {
		...
		while (s <= e) {
			...
			a.push(dateObj);
			s = new Date(s.setDate(s.getDate() + defaultOrderBy[reportBy]));
		}
		return a;
	}
	...
	function getDataArray(ordersStates, types) {
		...
		return dateRangs.map(function (getDatesRang) {
			...
			if ('day' === reportBy && ordersStatesApiData[0][rangeDate]) {
				...
				'week' === reportBy &&
				ordersStatesApiData[0][rangeDate]
			) {
				isCorrectRange = 1;
				...
				<ScFlex>
					...
					<div>
						<ScDropdown placement="bottom-end">
							...
							<ScMenu>
								<ScMenuItem onClick={() => setReportBy('day')}>
									{__('Daily', 'surecart')}
								</ScMenuItem>
								<ScMenuItem onClick={() => setReportBy('week')}>
									{__('Weekly', 'surecart')}
								</ScMenuItem>
								<ScMenuItem
									onClick={() => setReportBy('month')}
								>
									{__('Monthly', 'surecart')}
								</ScMenuItem>
								<ScMenuItem onClick={() => setReportBy('year')}>
									{__('Yearly', 'surecart')}
								</ScMenuItem>
							</ScMenu>
						</ScDropdown>
