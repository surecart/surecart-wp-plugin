...
export default () => {
	...
	const onCustomerDelete = async () => {
		const r = confirm(
			sprintf(
				__(
					'Warning: Deleting "%s" will permanently remove all associated data, including orders and subscriptions within a day. This action cannot be undone. Are you sure you want to continue?',
					'surecart'
				),
				customer?.name ||
					customer?.email ||
