 ...
 */
import { buildTermsTree } from './build-terms-tree';
...
export function HierarchicalTermSelector({ slug }) {
	...
	const newTermButtonLabel = labelWithFallback(
		'add_new_item',
		__('Add new category', 'surecart'),
		__('Add new term', 'surecart')
	);
	...
	const parentSelectLabel = labelWithFallback(
		'parent_item',
		__('Parent Category', 'surecart'),
		__('Parent Term', 'surecart')
	);
	const noParentOption = `— ${parentSelectLabel} —`;
	const newTermSubmitLabel = newTermButtonLabel;
	const filterLabel =
		taxonomy?.labels?.search_items ?? __('Search Terms', 'surecart');
	const groupLabel = taxonomy?.name ?? __('Terms', 'surecart');
			...
			{showForm && (
				...
				>
					<Flex direction="column" gap="4">
						...
						<FlexItem>
							<ScButton submit>{newTermSubmitLabel}</ScButton>
						</FlexItem>
					</Flex>
				</ScForm>
			)}
