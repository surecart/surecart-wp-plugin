...
export default ({ checkout, busy, loading }) => {
	...
	const [shippingId, setShippingId] = useState(checkout?.selected_shipping_choice);
    ...
    return (
        <Box title={__('Shipping', 'surecart')} loading={loading}>
            <ScRadioGroup
                onScChange={(e) => setShippingId(e.target.value)}
            >
                
                {(checkout?.shipping_choices?.data || []).map(({ id, amount, currency, shipping_method }, index) => {
                    const isLastElement = index !== checkout?.shipping_choices?.data?.length - 1;
                    return (
                        <>
                        <ScRadio 
                            key={id} 
                            value={id} 
                            checked={shippingId? shippingId === id : checkout?.selected_shipping_choice === id}
                            style={{
                                padding: '14px',
                                borderBottom: isLastElement ? '1px solid #eaeaea' : ''
                            }}
                        >
                            <div
                                style={{ 
