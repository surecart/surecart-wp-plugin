...
```php
add_filter('checkout_engine/checkout/validate', 'my_custom_validation', 10, 3);
...
the users input and transform or change it before being added to the order when it's submitted. This is run after all validation has passed. Any validation should occur in the `checkout_engine/checkout/validate` filter.

```php
add_filter('checkout_engine/checkout/submit', 'my_custom_dynamic_field', 10, 2);
...
```php
add_action('checkout_engine/checkout/complete', 'my_custom_action', 10, 2);
...
```php
add_action('checkout_engine/checkout/failed', 'my_custom_failure_action', 10, 2);
