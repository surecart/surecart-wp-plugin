...
Checkout forms are rendered and processed using the `presto-checkout-form` component. This component serializes both Presto form controls and native form controls located inside when the form is submitted.
...
To add a form field to a checkout form, or all checkout forms you should use one of the following filters:

```php
add_filter('presto_pay/checkout/fields/coupon', 'your_custom_function');
add_filter('presto_pay/checkout/fields/billing', 'your_custom_function');
add_filter('presto_pay/checkout/fields/shipping', 'your_custom_function');
add_filter('presto_pay/checkout/fields/payment', 'your_custom_function');
```

## Add A Field

Many times you'll want to collect additional billing information from the user. To do this, you can use the `presto_pay/checkout/fields/billing` filter to add your own field.

```php
add_filter('presto_pay/checkout/fields/billing', 'my_custom_field', 10, 2);

function my_custom_field($fields, $checkout_form_id) {


    $fields['my_custom_id'] = [
        'label'         => 'Phone', // the name for your field
        'required'      => true, // whether it's required
        'type'          => 'tel', // the field type
		'validate'      => [ // client side validation using the Contraint Validation API
            'pattern' => '[0-9]{3}'
        ],
        'part'          => 'my-phone', // allows it to be styled via presto-pay::part(my-phone)
        'autocomplete'  => 'tel', // autocomplete rules
		'priority'      => 100, // where to show it priority-wise
    ];
