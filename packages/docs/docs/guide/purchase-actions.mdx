...
function provide_access_to_membership( $purchase_id ) {
	...
	if ( empty( $memberships[ $purchase->product ] ) ) {
		return;
	}
	...
	if ( $wp_user ) {
		...
		update_user_meta( $wp_user->ID, $memberships[ $purchase->product ], true );
	}
}
```
...
function remove_access_to_membership( $purchase_id ) {
	...
	if ( $wp_user ) {
		...
		delete_user_meta( $wp_user->ID, $memberships[ $purchase->product ] );
	}
}
```
...
// this action runs when a user's purchase is revoked. This can occur on refunds or subscription cancelations.
add_action( 'checkout_engine/purchase_updated', 'update_access_to_membership', 10, 1 );

// Use the order_paid event to update the users meta
function update_access_to_membership( $purchase_id ) {
	// fetch order with line items so we can get the purchased price_id.
	$purchase = Purchase::find( $purchase_id );
	...
	// get the purchase's previous quantity and product.
	[ 'product' => $product ] = (array) $purchase->toArray();
	[ 'product' => $previous_product ] = (array) $purchase->previous_attributes;
	...
	if ( $wp_user ) {
		...
		delete_user_meta( $wp_user->ID, $memberships[ $previous_product ] );
		// update access to new.
		update_user_meta( $wp_user->ID, $memberships[ $product ], true );
	}
}
```
