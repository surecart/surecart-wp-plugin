...
SureCart provides a convenient way to provide access after purchase. This is done through the
...
function provide_access_to_membership( \SureCart\Models\Purchase $purchase ) {
...
function remove_access_to_membership( \SureCart\Models\Purchase $purchase ) {
	// we only want to proceed if the purchase is for one of our products
	if ( empty( $memberships[ $purchase->product_id ] ) ) {
		return;
	}
...
// Use the order_paid event to update the users meta
function update_access_to_membership( \SureCart\Models\Purchase $purchase ) {
	// we only want to proceed if the purchase is for one of our products
