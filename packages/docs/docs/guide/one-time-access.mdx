...
// Use the order_paid event to update the users meta
add_action( 'checkout_engine/refund_succeeded', function( $order_id ) {
	// fetch order with line items.
	$refund = Refund::with( ['charge', 'charge.order', 'order.line_items'] )->find( $order_id );

	// check if the charge has been fully refunded.
	if( !$refund->charge->fully_refunded ) {
		return;
	}

	// we only want to proceed if the order contains the membership_price_id.
	if( ! $refund->charge->order->hasPriceId( $membership_price_id ) ) {
		return;
	}
