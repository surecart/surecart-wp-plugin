...
Web components have an additional benefit of "Style encapsulation" via [Shadow Dom](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM). This prevents other styles on a specific page from inheriting into these components, so they are safe to use no matter what styles exist on the page.
