...
// Enable enable on paragraph and button blocks.
const enableToolbarButtonOnBlocks = ['core/paragraph', 'core/button'];
...
const withToolbarButtonProp = createHigherOrderComponent((BlockListBlock) => {
	return (props) => {
		...
		if (paragraphAttribute && 'custom' === paragraphAttribute) {
			return (
				<BlockListBlock {...props} className={'has-custom-attribute'} />
			);
		} else {
			return <BlockListBlock {...props} />;
		}
