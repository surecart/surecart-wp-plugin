...
test.describe('Product', () => {
  ...
  test.beforeAll(async ({ requestUtils, admin }) => {
    ...
    collection1 = await createProductCollection(requestUtils, 'Collection 1');
    collection2 = await createProductCollection(requestUtils, 'Collection 2');
  ...
  test('Product page - Product List', async ({ page }) => {
    ...
    await expect(await page.locator('sc-product-item-title').nth(0).innerText()).toBe('Product 2');
    await expect(await page.locator('sc-product-item-title').nth(1).innerText()).toBe('Product 1');
    ...
    await page.locator('.product-item-list__sort sc-dropdown').nth(0).click();
    await page.click('text=Alphabetical, Z-A');
    await page.waitForResponse(resp => resp.url().includes('surecart/v1/products'));
    ...
    await expect(await page.locator('sc-product-item-title').nth(0).innerText()).toBe('Product 1');
    await expect(await page.locator('sc-product-item-title').nth(1).innerText()).toBe('Product 2');

    // Test: if Product 1 and Product 2 are showing in the list.
    await expect(await page.locator('sc-format-number').nth(0).innerText()).toBe('$10');
    await expect(await page.locator('sc-format-number').nth(1).innerText()).toBe('$20');

    // Test: Product image.
    // Check if Product 1 is showing in the list with the image.
    await expect(await page.locator('sc-product-item-image .product-img img').nth(0).getAttribute('src')).toBe('https://placehold.co/600x400/EEE/31343C');
    await expect(await page.locator('sc-product-item-image .product-img img').nth(1).getAttribute('src')).not.toBeNull();

    // Test: filter by product collection.
    await page.locator('.product-item-list__sort sc-dropdown').nth(1).click();
    ...
    await expect(await page.locator('sc-product-item-title').nth(0).innerText()).toBe('Product 1'); // As already in Ascending order.
    await expect(await page.locator('sc-product-item-title').nth(1).innerText()).toBe('Product 2');
  });
  ...
  test('Product page - Product Collection', async ({ page }) => {
    ...
    await expect(page.getByRole('heading').getByText('Collection 1', { exact: true })).toBeVisible();

    // Check if Product 1 are showing in the list.
    await expect(page.locator('sc-product-item-title').nth(0).getByText('Product 1', { exact: true })).toBeVisible();
  });
});
...
export const createProductCollection = async (requestUtils, collectionName) => {
  try {
    ...
    if (error?.code === 'product_collection.invalid') {
      const collections = await requestUtils
        .rest({
          method: 'GET',
