import { newSpecPage } from '@stencil/core/testing';
import { PrestoSecureNotice } from '../presto-secure-notice';

describe('presto-secure-notice', () => {
  it('renders', async () => {
    const page = await newSpecPage({
      components: [PrestoSecureNotice],
      html: `<presto-secure-notice></presto-secure-notice>`,
    });
    expect(page.root).toEqualHtml(`
      <presto-secure-notice>
        <mock:shadow-root>
          <div class="notice" part="base">
            <svg class="notice__icon" fill="currentColor" height="16" part="icon" viewBox="0 0 512 512" width="16" xmlns="http://www.w3.org/2000/svg">
              <title>
                ionicons-v5-g
              </title>
              <path d="M368,192H352V112a96,96,0,1,0-192,0v80H144a64.07,64.07,0,0,0-64,64V432a64.07,64.07,0,0,0,64,64H368a64.07,64.07,0,0,0,64-64V256A64.07,64.07,0,0,0,368,192Zm-48,0H192V112a64,64,0,1,1,128,0Z"></path>
            </svg>
            <span class="notice__text" part="text">
              <slot>
                This is a secure, encrypted payment.
              </slot>
            </span>
          </div>
        </mock:shadow-root>
      </presto-secure-notice>
    `);
  });
});
