...
describe('ce-button', () => {
  ...
  it('renders', async () => {
    ...
    await page.setContent('<ce-button></ce-button>');

    const element = await page.find('ce-button');
  ...
  it('Can be loading', async () => {
    ...
    await page.waitForChanges();
    const spinner = await page.find(`${selector} >>> ce-spinner`);
