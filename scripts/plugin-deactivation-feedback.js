(function ($) {
	const UserDeactivationPopup = {
		...
		_hideShowFeedbackAndCTA(event) {
			...
			const showCta = $(event.target).attr('data-show_cta') === 'true';
		...
		_submitFeedback(event, self) {
			...
			// Check if the button action is 'skip' and skip sending survey & deactivate plugin.
			if ('skip' === this.buttonAction) {
				this._redirectOrReload();
				return;
			}
			...
			const formData = {
				...
				referer: closestForm.find('input[name="referer"]').val() || '',
				version: closestForm.find('input[name="version"]').val() || '',
